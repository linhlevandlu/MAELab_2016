...
void ImageViewer::approximatedLines()
{
	...
	vector<ptr_Line> lines = matImage->getApproximateLines(3.5);
	/*RGB color;
	 color.R = 255;
	 color.G = color.B = 0;
	 ptr_Line li;
	 vector<ptr_Point> dPoints;
	 ptr_Point dp;
	 for (size_t i = 0; i < lines.size(); i++) {
	 li = lines.at(i);
	 dPoints = drawingLine(li,color);
	 for (size_t k = 0; k < dPoints.size(); ++k) {
	 dp = dPoints.at(k);
	 matImage->getRGBMatrix()->setAtPosition(dp->getY(), dp->getX(),
	 dp->getColor());
	 }
	 }*/
	this->loadImage(matImage, ptrRGBToQImage(matImage->getRGBMatrix()),
